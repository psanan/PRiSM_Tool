...
# FIXME: This (among other functions) is not written in the most compact way, because we lack nicely-wrapped "c2f" functions
# That should be remedied for the 3d extension.
function dvx_dy(tree::SimpleTreeMesh, c_id, problem)
    ...
    if corner_type == corner_boundary
        if corner_info & 0b1100 == 0b1100
            # Note that this domain corner case is not actually required for free-slip or Dirichlet BCs
            vx_boundary =  problem.vx_ref(get_corner_coordinates(tree, c_id)...)
            if corner_info == CORNER_BOUNDARY_DOWN_LEFT
                e_id_up_right = tree.c2e[UP_RIGHT, c_id]
                f_id_up = tree.e2f[LEFT, e_id_up_right]
                h_up = get_element_size(tree, e_id_up_right)/2
                cols = f_id_to_eqnum(tree, [f_id_up])
                vals =  [1.0/h_up]
                rhs_val = vx_boundary / h_up
            elseif corner_info == CORNER_BOUNDARY_DOWN_RIGHT
                e_id_up_left = tree.c2e[UP_LEFT, c_id]
                f_id_up = tree.e2f[RIGHT, e_id_up_left]
                h_up = get_element_size(tree, e_id_up_left)/2
                cols = f_id_to_eqnum(tree, [f_id_up])
                vals =  [1.0/h_up]
                rhs_val = vx_boundary / h_up
            elseif corner_info == CORNER_BOUNDARY_UP_LEFT
                e_id_down_right = tree.c2e[DOWN_RIGHT, c_id]
                f_id_down = tree.e2f[LEFT, e_id_down_right]
                h_down = get_element_size(tree, e_id_down_right)/2
                cols = f_id_to_eqnum(tree, [f_id_down])
                vals = [-1.0/h_down]
                rhs_val = -1.0 * vx_boundary / h_down
            else
                @assert corner_info == CORNER_BOUNDARY_UP_RIGHT
                e_id_down_left = tree.c2e[DOWN_LEFT, c_id]
                f_id_down = tree.e2f[RIGHT, e_id_down_left]
                h_down = get_element_size(tree, e_id_down_left)/2
                cols = f_id_to_eqnum(tree, [f_id_down])
                vals = [-1.0/h_down]
                rhs_val = -1.0 * vx_boundary / h_down
            end
            ...
            if corner_type_masked in [CORNER_BOUNDARY_DOWN_MASK, CORNER_BOUNDARY_UP_MASK]
                ...
                if corner_type_masked == CORNER_BOUNDARY_LEFT_MASK
                    ...
                    f_id_down = tree.e2f[LEFT, e_id_down_right]
                    h_down = get_element_size(tree, e_id_down_right)/2
                elseif corner_type_masked == CORNER_BOUNDARY_RIGHT_MASK
                    e_id_up_left = tree.c2e[UP_LEFT, c_id]
                    f_id_up = tree.e2f[RIGHT, e_id_up_left]
                    h_up = get_element_size(tree, e_id_up_left)/2
                    e_id_down_left = tree.c2e[DOWN_LEFT, c_id]
...
function dvy_dx(tree::SimpleTreeMesh, c_id, problem)
    ...
    if corner_type == corner_boundary
        if corner_info & 0b1100 == 0b1100
            ...
            if corner_info == CORNER_BOUNDARY_DOWN_LEFT
                e_id_up_right = tree.c2e[UP_RIGHT, c_id]
                f_id_right = tree.e2f[DOWN, e_id_up_right]
                h_right = get_element_size(tree, e_id_up_right) / 2
                cols = f_id_to_eqnum(tree, [f_id_right])
                vals = [1.0/h_right]
                rhs_val = vy_boundary / h_right
            elseif corner_info == CORNER_BOUNDARY_DOWN_RIGHT
                e_id_up_left = tree.c2e[UP_LEFT, c_id]
                f_id_left = tree.e2f[DOWN, e_id_up_left]
                h_left = get_element_size(tree, e_id_up_left) / 2
                cols = f_id_to_eqnum(tree, [f_id_left])
                vals = -1.0 / h_left
                rhs_val = -1.0 * vy_boundary / h_left
            elseif corner_info == CORNER_BOUNDARY_UP_LEFT
                e_id_down_right = tree.c2e[DOWN_RIGHT, c_id]
                f_id_right = tree.e2f[UP, e_id_down_right]
                h_right = get_element_size(tree, e_id_down_right) / 2
                cols = f_id_to_eqnum(tree, [f_id_right])
                vals = [1.0/h_right]
                rhs_val = vy_boundary / h_right
            else
                @assert corner_info == CORNER_BOUNDARY_UP_RIGHT
                e_id_down_left = tree.c2e[DOWN_LEFT, c_id]
                f_id_left = tree.e2f[UP, e_id_down_left]
                h_left = get_element_size(tree, e_id_down_left) / 2
                cols = f_id_to_eqnum(tree, [f_id_left])
                vals = -1.0 / h_left
                rhs_val = -1.0 * vy_boundary / h_left
            end
            ...
            if corner_type_masked in [CORNER_BOUNDARY_LEFT_MASK, CORNER_BOUNDARY_RIGHT_MASK]
                ...
                if corner_type_masked == CORNER_BOUNDARY_DOWN_MASK
                    ...
                    f_id_right = tree.e2f[DOWN, e_id_up_right]
                    h_right = get_element_size(tree, e_id_up_right)/2
                elseif corner_type_masked == CORNER_BOUNDARY_UP_MASK
                    e_id_down_left = tree.c2e[DOWN_LEFT, c_id]
                    f_id_left = tree.e2f[UP, e_id_down_left]
                    h_left = get_element_size(tree, e_id_down_left) / 2
                    e_id_down_right = tree.c2e[DOWN_RIGHT, c_id]
...
function assemble_system(tree::SimpleTreeMesh, problem, h_min; Txy_method=1, zero_diri_vel_cols=false, nonzero_x0=false, modified_scaling=false, modified_stress=false)
    ...
    for e_id = 1:tree.ne
        ...
        if e_id == 1
            # Fix single pressure node
            push!(row, eqnum)
            push!(col, eqnum)
            push!(val, Kbnd)
            x, y = get_element_coordinates(tree, e_id)
            b[eqnum] = (Kbnd / Kcont) * problem.p_ref(x, y)
        else
